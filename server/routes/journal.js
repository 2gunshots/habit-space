...
router.post("/entry", async (req, res) => {
    ...
    try {
        const prevEntry = await db.query(
            "SELECT journal_id FROM journals where date = CURRENT_DATE AND user_id = $1;",
            [userId]
        );
        if (prevEntry.rows.length > 0) {
            //entry exists
            const updateJournal = `UPDATE journals
            ...
            const createJournal = `INSERT INTO journals(content, date, user_id)
