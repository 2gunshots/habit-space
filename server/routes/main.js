...
router.get("/", authMiddleware, async (req, res) => {
    try {
        const allids = await db.query("SELECT habit_id FROM habits");
...
const midnightStreakUpdate = async (habitId) => {
    try {
        const query = `SELECT streak, last_log FROM habits WHERE habit_id = $1`;
        ...
        if (streak > 0) {
            if (differenceInDays > 1) {
                newStreak = 0;
                const streakQuery = `UPDATE habits SET streak = $1 WHERE habit_id = $2;`;
